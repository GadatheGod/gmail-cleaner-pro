...
async def api_preview_status():
    ...
    try:
        ...
        logger.exception("Error getting preview status")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/preview-results")
async def api_preview_results():
    """Get preview results."""
    try:
        return get_preview_results()
    except Exception as e:
        logger.exception("Error getting preview results")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get preview results",
        ) from e
