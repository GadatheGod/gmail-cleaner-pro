...
def _save_token(token_json: str) -> bool:
    """Robustly save token to file, handling Docker filesystem issues.

    Args:
        token_json: JSON string of the token to save.

    Returns:
        True if save succeeded, False otherwise.
    """
    token_dir = os.path.dirname(settings.token_file) or "."
    if not os.path.isdir(token_dir):
        try:
            os.makedirs(token_dir, exist_ok=True)
        except OSError as e:
            print(f"ERROR: Cannot create token directory at {token_dir}: {e}")
            logger.error(f"Failed to create token directory {token_dir}: {e}", exc_info=True)
            return False

    if not os.access(token_dir, os.W_OK):
        print(f"ERROR: Token directory is not writable: {token_dir}")
        logger.error(f"Token directory is not writable: {token_dir}")
        return False
    ...
    try:
        ...
        print(f"ERROR: Failed to save token file at {settings.token_file}: {e}")
        logger.error(f"Failed to save token file {settings.token_file}: {e}", exc_info=True)
        return False
...
def _try_refresh_creds(creds: Credentials) -> Credentials | None:
    ...
    try:
        ...
        if not _save_token(creds.to_json()):
            pass
        return creds
        ...
        if not creds or not creds.valid:
            ...
            def run_oauth() -> None:
                try:
                    ...
                    if not _save_token(new_creds.to_json()):
                        raise OSError(f"Failed to save token file at {settings.token_file}")
